    ...
    public class AuthorizeAttribute : FilterAttribute, IAuthorizationFilter
    {
        ...
        protected void HandleUnauthorizedRequest(ref AuthorizationContext filterContext)
        {
            ...
            if (!this.CheckSessionExpired(filterContext.HttpContext))
            {
                if (isAjaxRequest)
                {
                    filterContext.Result = setResult(VsixMvcAppResult.Resources.General.GeneralTexts.SessionExpired);
                }
                else
                {
            ...
            if (!this.CheckIsAutohrized(filterContext.HttpContext))
            {
                if (isAjaxRequest)
                {
                    filterContext.Result = setResult(VsixMvcAppResult.Resources.General.GeneralTexts.PermissionDenied);
                }
                else
                {
