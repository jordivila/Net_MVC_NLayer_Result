...
namespace VsixMvcAppResult.UI.Web.Common.Mvc.Attributes
{
    public class HandleErrorAttributeExtended : HandleErrorAttribute
    {
        public override void OnException(ExceptionContext filterContext)
        {
            ...
            LoggingHelper.Write(new LogEntry(filterContext.Exception, LoggerCategories.UIServerSideUnhandledException.ToString(), 1, 1, TraceEventType.Error, LogTitle, param));
            ...
            if (filterContext.RequestContext.HttpContext.Request.IsAjaxRequest())
            {
                JsonResult jr = new JsonResult()
                {
                    Data = new DataResultString()
                                    {
                                        Message = VsixMvcAppResult.Resources.General.GeneralTexts.UnexpectedError,
                                        IsValid = false,
                                        Data = VsixMvcAppResult.Resources.General.GeneralTexts.UnexpectedError,
