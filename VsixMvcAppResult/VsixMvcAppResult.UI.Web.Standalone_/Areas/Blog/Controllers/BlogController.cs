...
using VsixMvcAppResult.UI.Web.Areas.Blog.Models;
using VsixMvcAppResult.UI.Web.Controllers;
using VsixMvcAppResult.UI.Web.Common.Mvc.Html;

namespace VsixMvcAppResult.UI.Web.Areas.Blog.Controllers
{
    public class BlogController : Controller, IControllerWithClientResources
    {
        ...
        private BlogModel GetIndexModel()
        {
            ...
            model.FeedFilter = new DataFilterSyndication() { Page = 0, PageSize = (int)VsixMvcAppResult.Models.Enumerations.PageSizesAvailable.RowsPerPage10 };
        ...
        private BlogModel GetIndexModelByTitle(string title)
        {
            ...
            model.FeedId = string.Empty;
            model.FeedFormatter = this.ProviderSyndication.GetInfoWithNoItems();
            model.FeedFilter = new DataFilterSyndication() { Uri = title, Page = 0, PageSize = (int)VsixMvcAppResult.Models.Enumerations.PageSizesAvailable.RowsPerPage10 };
            ...
            if (this.RequestType() == HttpVerbs.Get)
            {
                model = new BlogModel()
                {
                    ...
                    FeedFilter = new DataFilterSyndication()
                    {
                        ...
                        Page = 0,
                        PageSize = (int)VsixMvcAppResult.Models.Enumerations.PageSizesAvailable.RowsPerPage10
                    }
                };
            }
            else
